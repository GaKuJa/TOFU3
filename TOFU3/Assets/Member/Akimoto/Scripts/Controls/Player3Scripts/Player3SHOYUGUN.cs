...
public class Player3SHOYUGUN : BaseGunStatus
{
    public static Player3SHOYUGUN Instance { get => _instance; }
    static Player3SHOYUGUN _instance;
    // ゲームオブジェクトプレハブ
    [SerializeField]
    ...
    void Update()
    {
        ...
        if (Input.GetAxis("L_R_Trigger3") > 0)
        {
            ShotPrefab();
            shootingFlag = true;
        }
        if (Input.GetKeyDown("joystick 3 button 2") && magazinNum != 0)
            ReloadBullet();
        if (shootingFlag)
            IntervalControl();
    }
