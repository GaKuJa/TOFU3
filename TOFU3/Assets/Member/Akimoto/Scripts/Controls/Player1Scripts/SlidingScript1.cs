...
public class SlidingScript1 : MonoBehaviour
{
    private float startYScale;  // 初期の高さ
    private float slideTimer;   // スライディング時間
    private Rigidbody rb;       // 物理
    private Player1ControllerScript pm; // MovementManagerを取得(元がPlayerMovveだったので変数はpm)
    ...
    void Start()
    {
        ...
        pm = GetComponent<Player1ControllerScript>();   // MovementManagerを取得
