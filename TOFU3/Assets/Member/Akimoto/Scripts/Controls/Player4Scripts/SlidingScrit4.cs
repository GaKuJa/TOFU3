...
public class SlidingScrit4 : MonoBehaviour
{
    private float startYScale;  // 初期の高さ
    private float slideTimer;   // スライディング時間
    private Rigidbody rb;       // 物理
    private Player4ControllerScript pm; // MovementManagerを取得(元がPlayerMovveだったので変数はpm)
    ...
    void Start()
    {
        ...
        pm = GetComponent<Player4ControllerScript>();   // MovementManagerを取得
