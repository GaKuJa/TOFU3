...
public class Player4SHOYUGUN : BaseGunStatus
{
    public static Player4SHOYUGUN Instance { get => _instance; }
    static Player4SHOYUGUN _instance;
    // ゲームオブジェクトプレハブ
    [SerializeField]
    ...
    void Update()
    {
        ...
        if (Input.GetAxis("L_R_Trigger4") > 0)
        {
            ShotPrefab();
            shootingFlag = true;
        }
        if (Input.GetKeyDown("joystick 4 button 2") && magazinNum != 0)
            ReloadBullet();
        if (shootingFlag)
            IntervalControl();
    }
