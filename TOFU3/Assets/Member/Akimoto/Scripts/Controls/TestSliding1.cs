...
public class TestSliding1 : MonoBehaviour
{
    private float startYScale;  // 初期の高さ
    private float slideTimer;   // スライディング時間
    private Rigidbody rb;       // 物理
    private TestPlayer1Controler pm; // MovementManagerを取得(元がPlayerMovveだったので変数はpm)
    ...
    void Start()
    {
        ...
        pm = GetComponent<TestPlayer1Controler>();   // MovementManagerを取得
