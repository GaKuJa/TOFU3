...
public class TestSliding2 : MonoBehaviour
{
    private float startYScale;  // 初期の高さ
    private float slideTimer;   // スライディング時間
    private Rigidbody rb;       // 物理
    private TestPlayer2Controler pm; // MovementManagerを取得(元がPlayerMovveだったので変数はpm)
    ...
    void Start()
    {
        ...
        pm = GetComponent<TestPlayer2Controler>();   // MovementManagerを取得
        pm = GetComponent<TestPlayer2Controler>();   // MovementManagerを取得
